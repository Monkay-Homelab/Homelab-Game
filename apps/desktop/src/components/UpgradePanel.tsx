...
import { CURRENCY_COLORS, getCostCurrencyId } from '../utils/currencyColors';
          ...
          <div key={type_} className="panel p-4 flex flex-col min-h-0 overflow-hidden">
            <div className="flex items-center justify-between mb-3 shrink-0">
              <h3 className="text-sm font-semibold" style={{ color: cfg.color }}>
                ...
                {type_ === 'cooling' && (
                  <span
                    className="font-mono font-normal text-xs ml-1"
                    style={{ color: state.overheating ? 'var(--accent-red)' : 'var(--text-muted)' }}
                  >
                  ...
                  <div key={u.name} className="panel-card p-3 flex items-center justify-between">
                    <div className="min-w-0">
                      <div className="font-medium text-sm truncate">
                        ...
                        {u.persistent && (
                          <span className="text-xs ml-1" style={{ color: 'var(--accent-green)' }}>
                            (kept)
                          </span>
                        )}
                      </div>
                    ...
                    {owned ? (
                      ...
                      (() => {
                        const costCurrency = CURRENCY_COLORS[getCostCurrencyId(u.cost_type)];
                        const canAfford =
                          u.cost_type === 'money'
                            ? state.money >= u.cost
                            : state.compute_units >=
                              Math.floor(u.cost * (u.type === 'automation' ? scale : 1));
                        return (
                          <button
                            ...
                            style={{
                              background: canAfford ? costCurrency.bg : 'var(--bg-card)',
                              color: canAfford ? costCurrency.color : 'var(--text-muted)',
                              border: `1px solid ${canAfford ? costCurrency.border : 'var(--border)'}`,
                            }}
                          >
                            {u.cost_type === 'money'
                              ? `$${u.cost.toLocaleString()}`
                              : `${Math.floor(u.cost * (u.type === 'automation' ? scale : 1)).toLocaleString()} CU`}
                          </button>
                        );
                      })()
                    )}
