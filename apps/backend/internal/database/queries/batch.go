...
// LoadFullGameStateForUpdate loads all game state data for a user within a
// transaction, acquiring a row-level lock (FOR UPDATE) on the game_states row.
// This provides database-level serialization for read-modify-write cycles,
// preventing concurrent transactions from reading stale state.
//
// The caller must have already begun a transaction via pool.Begin() and must
...
func LoadFullGameStateForUpdate(ctx context.Context, tx pgx.Tx, userID string) (*FullGameData, error) {
	// Phase 1: Get game_state by user_id with FOR UPDATE lock
	var gs models.GameState
	err := tx.QueryRow(ctx,
		`SELECT `+gsColumns+` FROM game_states WHERE user_id = $1 FOR UPDATE`, userID,
	).Scan(gsFields(&gs)...)
	if err != nil {
		return nil, fmt.Errorf("batch: failed to load game_state for update: %w", err)
	}

	// Phase 2: Batch all 8 child table reads in a single network round-trip.
	// Child rows are implicitly protected by the parent FOR UPDATE lock — any
	// concurrent transaction must first acquire the game_states lock, which
