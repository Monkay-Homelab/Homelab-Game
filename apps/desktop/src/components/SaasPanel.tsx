...
export function SaasPanel({ state }: { state: GameState }) {
  ...
  if (!isRack) {
    return (
      <div className="h-full panel p-6 flex items-center justify-center">
        <p className="font-mono text-sm" style={{ color: 'var(--text-muted)' }}>Reach a rack tier to unlock SaaS</p>
      </div>
    );
  }
    ...
    return (
      <div className="h-full panel p-6 flex flex-col items-center justify-center">
        ...
        <button
          ...
          style={{
            background: canUnlock ? 'rgba(245,158,11,0.1)' : 'var(--bg-card)',
            color: canUnlock ? 'var(--accent-amber)' : 'var(--text-muted)',
            border: `1px solid ${canUnlock ? 'rgba(245,158,11,0.3)' : 'var(--border)'}`,
          }}
        >
          Unlock SaaS
        </button>
  ...
  const sortedCustomers = [...customers].sort((a, b) => (typePower[a.service_type] || 0) - (typePower[b.service_type] || 0));
    ...
    <div className="h-full flex gap-4 min-h-0">
      ...
      <div className="flex-1 panel p-4 flex flex-col min-h-0">
        <div className="flex justify-between items-center mb-3 shrink-0">
          <h3 className="text-sm font-semibold" style={{ color: 'var(--accent-amber)' }}>SaaS Services</h3>
              ...
              <div key={s.name} className="panel-card p-3 flex items-center justify-between">
                <div>
                  ...
                  <div className="font-mono text-xs mt-0.5" style={{ color: 'var(--text-secondary)' }}>
                    ${s.revenue_per_customer}/cust · {s.power_required}W
                  </div>
                </div>
                {deployed ? (
      ...
      <div className="flex-1 panel p-4 flex flex-col min-h-0">
        <div className="flex justify-between items-center mb-3 shrink-0">
          <h3 className="text-sm font-semibold" style={{ color: 'var(--accent-amber)' }}>Customers</h3>
          <span className="font-mono text-xs" style={{ color: netIncome >= 0 ? 'var(--accent-green)' : 'var(--accent-red)' }}>
            ${netIncome}/tick net
          </span>
        </div>
        <div className="space-y-2 overflow-y-auto min-h-0 flex-1">
          {sortedCustomers.length > 0 ? sortedCustomers.map(c => (
            <div key={c.id} className="panel-card p-3 flex justify-between items-center">
              <div>
                ...
                <div className="font-mono text-xs mt-0.5" style={{ color: 'var(--text-secondary)' }}>
                  {c.service_type} · ${c.monthly_revenue}/tick
                </div>
              </div>
              <span className="font-mono text-xs" style={{ color: c.satisfaction >= 80 ? 'var(--accent-green)' : c.satisfaction >= 50 ? 'var(--accent-amber)' : 'var(--accent-red)' }}>
                {c.satisfaction}%
              </span>
            </div>
          )) : (
            <p className="font-mono text-xs" style={{ color: 'var(--text-muted)' }}>Deploy a service to attract customers</p>
          )}
        </div>
      </div>
      ...
      {expenses.length > 0 && (
        <div className="w-64 shrink-0 panel p-4 flex flex-col min-h-0">
          <h3 className="text-sm font-semibold mb-3 shrink-0" style={{ color: 'var(--accent-red)' }}>Expenses</h3>
          <div className="space-y-2 overflow-y-auto min-h-0 flex-1">
            {expenses.map(e => (
              <div key={e.id} className="panel-card p-3 flex justify-between items-center">
                <span className="text-sm">{e.name}</span>
                <span className="font-mono text-xs" style={{ color: 'var(--accent-red)' }}>-${e.cost_per_tick}</span>
              </div>
            ))}
          </div>
        </div>
